...
def addString(x,  y,  str):
    framebuffer.text(str, x, y*8)

def text_wrap(str,x,y,w,h,border=None, color=1):
	# optional box border
	if border is not None:
		framebuffer.rect(x, y, w, h, border)
	cols = w // 8
	# for each row
	j = 0
	for i in range(0, len(str), cols):
		# draw as many chars fit on the line
		framebuffer.text(str[i:i+cols], x, y + j, color)
		j += 8
...
def drawBuffer():
    ...
    #i2c.writeto(SSD1306_I2C_ADDRESS, bytearray(0x40) + buffer)    
